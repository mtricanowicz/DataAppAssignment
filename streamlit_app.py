...
margin_calc=((df.where(df["Sub_Category"].isin(subcategory)).filter(items=['Profit']).dropna().sum()["Profit"])/(df.where(df["Sub_Category"].isin(subcategory)).filter(items=['Sales']).dropna().sum()["Sales"]))*100
