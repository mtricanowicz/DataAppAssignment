...
margin_calc=(df.where(df["Sub_Category"].isin(subcategory)).filter(items=['Profit']).dropna().sum()/df.where(df["Sub_Category"].isin(subcategory)).filter(items=['Sales']).dropna().sum())*100
